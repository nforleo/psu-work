...
test('s.subtract(t)', (t) => {
     ...
     *      10 (October 10, 2023)
     */
    t.equals(s.subtract(3, 'week').date(), 10, '(3, week)');

    /**
     * Test subtracting 3 hours
     *  input:
     *      hour: 9 (9:00am)
     *      num: 3
     *      unit: hour
     ...
     *      6 (6:00am)
     */
    t.equals(s.subtract(3, 'hour').hour(), 6, '(3, hour)');
    
    /**
     * Test subtracting 3 quarters
     *  input:
     *      quarter:    4 (4th quarter of the year)
     *      num:        1
     *      unit:       quarter
     *  expected output:
     *      1 (1st quarter of the previous year)
     */
    t.equals(s.subtract(3, 'quarter').quarter(), 1,'(3, quarter)')
    
    /**
     * Test subtracting 3 centuries
     *  input:
     *      year:   2023 (current year)
     *      num:    3
     *      unit:   century
     *  expected output:
     *      1723
     * This unit seems to be broken as the definition is hard coded to only subtract 100 years. It will fail.
     */
    t.equals(s.subtract(3, 'century').year(), 1723, '(3, century)'); 

    /**
     * Test subtracting a negative (-2 weeks)
     *  input:
     *      date:   31 (October 31, 2023)
     *      num:    -2
     *      unit:   week
     ...
     *      14 (November 14, 2023)
     */
    t.equals(s.subtract(-2, 'week').date(), 14, '(-2. Week)');

    /**
     * Test subtracting 0 weeks
     *  input:
     *      date:   31 (October 31, 2023)
     *      num:    0
     *      unit:   week
     ...
     *      31
     */
    t.equals(s.subtract(0, 'week').date(), 31,  '(0, week)');
...
// 12
test('.every()', (t) => {
    let start = spacetime('October 10th 2023', 'America/New_York')
    let end = spacetime('October 10th 2023', 'America/New_York').add(1, 'hour')
    
    /**
     * These tests were modified from the source testing. This was done durin
     *  explatory testing and easier to write as a automated test 
     * rather than verify manually. 
     */
    let days = start.every('day', end)
    t.equal(days.length, 1, '1 day')
    t.equal(days[0].timezone().name, 'America/New_York', 'results in right timezone')
  
    let weeks = start.every('week ', end)
    t.equal(weeks.length, 0, '0 weeks')
